...
const Auth = ({
  ...
  useEffect(() => {
    ...
    if (authType === KEYCLOAK) {
      ...
      keycloakTemp.login = options => {
        ...
        options2.idpHint = 'sso-insee';
        kcLogin(options2);
      };
      keycloakTemp.redirectUri = window.location.href.replace(window.location.search, '');
      setKc(keycloakTemp);
    }
  }, [authType]);
  ...
  return (
    <>
      {!error && (!authType || (authType === KEYCLOAK && !token)) && <div>Loading ...</div>}
