...
import { pathAvancement, pathTransmissionCampagne } from '../../utils/properties';
...
export default class TableauCollecte extends Component {
  static calculTotaux(donnees) {
    donnees[donnees.length] = {
      ...
      nbIntRecu: donnees.reduce((accumulator, vague) => accumulator + vague.nbIntRecu, 0),
      nbPapRecu: donnees.reduce((accumulator, vague) => accumulator + vague.nbPapRecu, 0),
      nbPND: donnees.reduce((accumulator, vague) => accumulator + vague.nbPND, 0),
      nbHC: donnees.reduce((accumulator, vague) => accumulator + vague.nbHC, 0),
      nbRefus: donnees.reduce((accumulator, vague) => accumulator + vague.nbRefus, 0),
      nbAutresDechets: donnees.reduce(
        (accumulator, vague) => accumulator + vague.nbAutresDechets,
        0
      ),
      nbIntPart: donnees.reduce((accumulator, vague) => accumulator + vague.nbIntPart, 0),
    };
    ...
    donnees[donnees.length] = {
      ...
      nbUe: 100,
      nbIntRecu: total === 0 ? 0 : (donnees[donnees.length - 1].nbIntRecu / total) * 100,
      nbPapRecu: total === 0 ? 0 : (donnees[donnees.length - 1].nbPapRecu / total) * 100,
      nbPND: total === 0 ? 0 : (donnees[donnees.length - 1].nbPND / total) * 100,
      nbHC: total === 0 ? 0 : (donnees[donnees.length - 1].nbHC / total) * 100,
      nbRefus: total === 0 ? 0 : (donnees[donnees.length - 1].nbRefus / total) * 100,
      nbAutresDechets:
        total === 0 ? 0 : (donnees[donnees.length - 1].nbAutresDechets / total) * 100,
      nbIntPart: total === 0 ? 0 : (donnees[donnees.length - 1].nbIntPart / total) * 100,
    };
